...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function showModal(message) {
        if (modal && modalMessage) {
            ...
            // 모달이 없으면 console.log로 처리
            console.log("Modal message:", message);
        }
    }

    // --- 3. 테이블 버튼 클릭 이벤트 (모달 사용) ---
    ...
    if (tableBody) {
        tableBody.addEventListener("click", (event) => {
            ...
            if (event.target.classList.contains("action-btn")) {
                ...
                if (action === "cancel") {
                    // '취소' 버튼 클릭 시
                    showModal(`'${programName}' 프로그램 신청을 취소하시겠습니까? (실제 취소 기능은 백엔드 구현이 필요합니다)`);
                    // 여기에 실제 취소 로직 (e.g., fetch API 호출) 추가 가능
                } else if (action === "register") {
                    // '산출물 등록' 버튼 클릭 시
                    showModal(`'${programName}' 프로그램 산출물을 등록합니다 (등록 페이지로 이동 또는 파일 업로드 로직)`);
