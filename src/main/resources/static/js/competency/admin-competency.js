document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeTree(treeData) {
        ...
        // 트리 [기능 추가] 선택 기능 활성화
        // 이제 줄이 클릭되면 tree.select() 함수가 작동할 것 입니다.
        tree.enableFeature('Selectable', {
            selectedClass: 'tui-tree-selected', // 선택될 때 붙을 클래스명
        });
...
// [수정된] 라이브러리 이벤트 대신 '수동 클릭 이벤트' 사용
        ...
        treeContainer.addEventListener('click', (e) => {
            ...
            // 1. 클릭된 요소에서 가장 가까운 부모 노드(트리 찾기
            // (TUI Tree에서 각 노드는 'tui-tree-node' 클래스를 가진 li 태그입니다.
            const treeNode = target.closest('.tui-tree-node');

            if (!treeNode) return; // 노드가 아니면 무시

            // 2. li 태그의 id 속성값이 바로 Node ID입니다. 바로 가져옵니다.
            const nodeId = treeNode.id;

            if (!nodeId) return;

            console.log('트리[수동 클릭] Node ID:', nodeId);

            // 3. UI 선택 효과 적용 (시각적 피드백)
            tree.select(nodeId);
            ...
            if (!node) {
                console.error('해당 노드 객체를 찾을 수 없습니다:', nodeId);
                return;
            }

            const nodeData = node.data || {};
            const realDbId = nodeData.competencyId; // 트리가 넘겨준 진짜 DB ID

            console.log('진짜 DB ID:', realDbId);

            // 5. 상세 정보 로딩
            if (realDbId) {
                fetchDetail(realDbId);
            } else {
                // 최상위 노드 등 data가 없는 경우를 대비해 id 사용
...
// 1-7. '하위 역량 추가' 버튼 (수정됨)
    document.getElementById('addChildButton').addEventListener('click', () => {
        // 1. 현재 선택된 트리 노드의 ID 가져오기
        const selectedNodeId = tree.getSelectedNodeId();

        if (!selectedNodeId) {
            alert('하위 역량을 추가하려면 위쪽 트리에서 먼저 선택하세요.');
            return;
        }

        // 2. 해당 ID를 사용해 노드 데이터 가져오기
        // (라이브러리 버전에 따라 getNodeData 또는 getNode 사용)
        const node = tree.getNodeData(selectedNodeId);

        // 3. 진짜 DB ID 추출
        // node.data.competencyId가 있으면 그것, 없으면 node.id가 숫자인지 확인해서 사용
        let realDbId = (node && node.data) ? node.data.competencyId : null;

        // [비상 대책] data 없이 있으면 최상위 레벨 등에서 node.id 자체가 DB ID일 수도 있음
        if (!realDbId && node.id && !isNaN(node.id)) {
            realDbId = node.id;
        }

        console.log('트리 [하위추가] 선택 ID:', selectedNodeId, '/ DB ID:', realDbId);

        if (!realDbId) {
            alert('선택된 노드의 DB ID를 찾을 수 없습니다. (콘솔 로그 확인 필요)');
            return;
        }

        // 4. 폼 열기 (실제 null이 아닌 진짜 부모가 들어갑니다)
        showDetailView(null, realDbId, true);

        // 트리 [추가] 작동 시 채우기(현재 자식 개수 + 1)
    ...
    saveButton.addEventListener('click', (e) => {
        ...
        saveButton.innerHTML = '<span class="spinner-border spinner-border-sm"></span> 저장중..';
    ...
    deleteButton.addEventListener('click', (e) => {
        ...
        if (!competencyId) return alert('삭제할 역량을 선택해 주세요.');
        ...
        fetch(`/competency-admin/competency/api/competencies/${competencyId}/questions`)
            ...
            .then(questions => {
                if (questions.length === 0) {
                    questionListBody.innerHTML = '<tr><td colspan="6" class="text-center py-4">등록된 문항이 없습니다.</td></tr>';
                    return;
                }

                // 각 QuestionListDto(JSON)를 HTML(tr)로 변환
    ...
    questionListBody.addEventListener('click', (e) => {
        ...
        if (e.target.classList.contains('btn-delete-question')) {
            ...
            fetch(`/competency-admin/competency/api/questions/${questionId}`, {
                ...
                .then(data => {
                    alert(data.message || '삭제되었습니다.');
                    row.remove(); // ⭐️ API 성공 후 화면에서 해당 행(tr) 즉시 제거
                })
                .catch(error => {
                    alert('삭제 실패: ' + error.message);
                    // ⭐️ (2) 실패 시 버튼 원상 복구
        ...
        if (target.classList.contains('btn-edit-question')) {
            ...
            // 1. (11단계 API 호출) 서버에서 '문항 상세정보 + 선택지 목록' DTO를 fetch
    ...
    document.getElementById('addNewQuestion').addEventListener('click', () => {
        ...
        if (!selectedCompetencyId) {
            alert("먼저 문항을 추가할 역량을 선택(클릭)해야 합니다.");
            return;
        }
    ...
    saveQuestionButton.addEventListener('click', (e) => {
        ...
        saveQuestionButton.innerHTML = '<span class="spinner-border spinner-border-sm"></span> 저장중..';
        ...
        // 2. QuestionFormDto에 맞게 폼 데이터를 수집
        const formData = {
    ...
    optionListContainer.addEventListener('click', (e) => {
        if (e.target.classList.contains('btn-delete-option')) {
            ...
            // (참고) 별도 updateOptionIndices는 불필요 함. 전송 시점에서 수집
        }
    });
