    ...
    try {
        if (type === 'line') {
            ...
            currentChart = new toastui.Chart.lineChart({
                ...
                options: { ...chartOptions, title: '점수 변화 추이 (꺾은선)' }
            });

        } else if (type === 'bar') {
            // 막대 차트 데이터 가공
            const barCategories = dataRows.map(row => row.assessmentTitle); // 진단명으로 일치
            ...
            currentChart = new toastui.Chart.barChart({
                ...
                options: { ...chartOptions, title: '점수 변화 추이 (막대)' }
            });
        }
    } catch (error) {
