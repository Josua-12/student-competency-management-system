document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadSections() {
        fetch('/admin/assessment-section/api/sections')
            ...
            .catch(err => {
                console.error(err);
                sectionListBody.innerHTML = `<tr><td colspan="6" class="text-danger">데이터를 불러오지 못했습니다.</td></tr>`
            });
    }
    ...
    function renderTable(sections) {
        ...
        sectionListBody.innerHTML = sections.map(section => {
            ...
            const isActive = section.isActive;    // DTO 필드명 확인
    ...
    /**
     * Helper: 모달 열기 (데이터 유무에 따라 등록/수정 모드 전환)
     */
